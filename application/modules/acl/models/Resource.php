	...
	*/
	...
	public function getAllowed( $role )
	{
		$select = $this->select()
			...
			->from( 
				array('rs'=> $this->_name), 
				array('id', 'module', 'controller', 'actioncontroller') 
			)
			->distinct()
			->joinInner( array('per' => Zend_Registry::get('tablePrefix').'acl_permission'), 'per.resource_id = rs.id', '' )
			->where('per.privilege=?', 'allow')
			->where('per.role_id=?', intval($role->id), Zend_Db::INT_TYPE )
		;
		#echo $select->__toString();
		#echo $this->fetchAll($select)->count();
		#die();
